...
typedef void (*serial_rx_callback)(serial_bus_h bus, const char *buffer, int length);
void serial_set_rx_callback(serial_bus_h bus, serial_rx_callback callback);
#endif

#if SERIAL_ASYNC_TX != OFF
bool serial_tx_ready(serial_bus_h bus);
void serial_write_async(serial_bus_h bus, void *buffer, int len);
#endif

void serial_bus_init(serial_bus_h bus, uint32_t baudrate);

int serial_read_byte(serial_bus_h bus, uint8_t *chr);
